...
import { api, type ApiOwnerApplication } from "../../services/api";
...
function statusBadge(status: ApiOwnerApplication["status"]) {
...
export default function OwnerApprovalsPage() {
  ...
  const [applications, setApplications] = useState<ApiOwnerApplication[]>([]);
  const [selected, setSelected] = useState<ApiOwnerApplication | null>(null);
  ...
  const loadApplications = useCallback(async () => {
    setLoading(true);
    try {
      const response = await api.admin.listOwnerApplications();
      setApplications(response.applications);
    } catch (error: unknown) {
      showError(error instanceof Error ? error.message : "Không thể tải hồ sơ Chủ ngựa.");
    } finally {
      setLoading(false);
    }
  }, [showError]);
  ...
  function openReview(application: ApiOwnerApplication) {
  ...
  async function review(status: "approved" | "rejected") {
    ...
    setReviewing(true);
    try {
      const response = await api.admin.reviewOwnerApplication(
        selected.id,
        status,
      ...
      showSuccess(status === "approved"
        ? "Đã duyệt và kích hoạt tài khoản Chủ ngựa."
        : "Đã từ chối hồ sơ Chủ ngựa.");
    } catch (error: unknown) {
      showError(error instanceof Error ? error.message : "Không thể xử lý hồ sơ Chủ ngựa.");
    } finally {
      setReviewing(false);
    }
  }
  ...
  return (
    <div className="page-stack">
      ...
      <Panel
        title="Hồ sơ Chủ ngựa chờ duyệt"
        subtitle="Kiểm tra hồ sơ PDF trước khi kích hoạt tài khoản Chủ ngựa"
        action={(
      ...
      >
        <DataTable
          rows={pending}
          loading={loading}
          empty="Không có hồ sơ Chủ ngựa nào đang chờ duyệt."
          columns={[
      ...
      {selected ? (
        <Panel
          title={`Xét duyệt Chủ ngựa - ${selected.fullName}`}
          subtitle="Quyết định duyệt sẽ kích hoạt tài khoản và cho phép Chủ ngựa đăng nhập"
          action={(
        ...
        >
          <div className="detail-grid">
            <div className="detail-item"><span className="detail-label">Họ và tên</span><strong>{selected.fullName}</strong></div>
            <div className="detail-item"><span className="detail-label">Email</span><strong>{selected.email}</strong></div>
            <div className="detail-item"><span className="detail-label">Số điện thoại</span><strong>{selected.phone || "—"}</strong></div>
            <div className="detail-item"><span className="detail-label">Ngày gửi</span><strong>{formatDate(selected.appliedAt)}</strong></div>
            <div className="detail-item"><span className="detail-label">Trạng thái</span><span>{statusBadge(selected.status)}</span></div>
          ...
          <div className="form-actions">
            ...
            <button className="primary-button" type="button" onClick={() => void review("approved")} disabled={reviewing}>
              {reviewing ? <><Spinner size="sm" onPrimary /> Đang xử lý...</> : "Duyệt Chủ ngựa"}
            </button>
          </div>
        </Panel>
      ) : null}

      <Panel title="Lịch sử xét duyệt" subtitle="Các hồ sơ Chủ ngựa đã được xử lý">
        <DataTable
          rows={reviewed}
          loading={loading}
          empty="Chưa có hồ sơ Chủ ngựa nào được xử lý."
          columns={[
            { key: "fullName", label: "Họ và tên" },
            { key: "email", label: "Email" },
